...
"""
Результат выполнения данной программы: 00 11 11 101 010 00 011 011 101 010 00 11 11 1000 1001
Результат, указанный в методичке:      00 11 11 101 011 00 010 010 101 011 00 11 11 1000 1001
Результаты не совпадают потому, что в методичке символы ' '(пробел) и 'o' поменены местами. 
В принципе, в данной фразе у нескольких элементов одинаковые частоты: 
- частота 2 у символов 'p', ' ', 'o'
- частота 1 у символов 'r' и '!'
Результат в методичке, я думаю, неверный. Потому что, получается, по алгоритму Хаффмана элементы должны располагаться
по убыванию частоты, а в случае одинаковой частоты двух и более элементов левее стоит тот элемент, 
который во фразе встречается раньше.
В таком случае, бинарное дерево получается вот таким:
                                    O
                        0 /                 \1
                         O                  O
                      0/    \1           0/   \1
